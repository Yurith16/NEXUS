...
const connectionOptions = {
  ...
  mobile: MethodMobile,
  browser: opcion === '1' ? ['*NEXUS*', 'Safari', '2.0.0'] : methodCodeQR ? ['*NEXUS*', 'Safari', '2.0.0'] : ['Ubuntu', 'Chrome', '20.0.04'],
  auth: {
    creds: state.creds,
...
conn.isInit = false;
conn.well = false;
conn.logger.info(`*[ 🜃 ] INICIANDO NEXUS...*\n`);
...
async function connectionUpdate(update) {
  ...
  if (connection == 'open') {
    console.log(chalk.yellow('*[ 🜃 ] Conexión establecida con el núcleo.*'));
    ...
    if (!global.subBotsInitialized) {
      ...
      try {
        ...
        console.error(chalk.red('*[ ⚡ ] Error al inicializar sub-núcleos:*'), error);
      }
    }
  }
  ...
  if (reason == 405) {
    console.log(chalk.bold.redBright(`*[ ⚡ ] Conexión replazada, reiniciando núcleo...*\n*Si persisten los errores, ejecuta: npm start*`));
  }
  if (connection === 'close') {
    if (reason === DisconnectReason.badSession) {
      ...
      if (shouldLogError('loggedOut')) {
        conn.logger.error(`*[ ⚡ ] Sesión cerrada, por favor elimina la carpeta ${global.authFile} y escanea nuevamente.*`);
      }
...
global.reloadHandler = async function (restatConn) {
  ...
  conn.welcome = '*👋 ¡Nuevo usuario detectado en el núcleo!*\n*@user*';
  conn.bye = '*👋 Usuario desconectado del núcleo.*\n*@user*';
  conn.spromote = '*[ ⚡ ] @user ascendido a administrador del núcleo.*';
  conn.sdemote = '*[ ⚡ ] @user degradado de administrador.*';
  conn.sDesc = '*[ ⚡ ] La descripción del núcleo ha sido modificada.*';
  conn.sSubject = '*[ ⚡ ] El nombre del núcleo ha sido actualizado.*';
  conn.sIcon = '*[ ⚡ ] Se ha actualizado la imagen del núcleo.*';
  conn.sRevoke = '*[ ⚡ ] El enlace de invitación ha sido restablecido.*';
        ...
        for (let i = 0; i < chatUpdate.messages.length; i++) {
          ...
          if (message?.key?.remoteJid?.endsWith('@g.us')) {
            try {
              ...
              // Verificar si hay menciones LID sin resolver
...
setInterval(async () => {
  ...
  const bio = `*⏱️ Activo: ${uptime} | 🕸️ NEXUS*`;
